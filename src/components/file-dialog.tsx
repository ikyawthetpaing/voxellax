      ...
      if (rejectedFiles.length > 0) {
        rejectedFiles.forEach(({ errors }) => {
          if (errors[0]?.code === "file-too-large") {
            toast({description:
              `File is too large. Max size is ${formatBytes(maxSize)}`, variant: "destructive"
            });
            return;
          }
          errors[0]?.message && toast({description: errors[0].message, variant: "destructive"});
        });
      }
    },
  ...
  return (
    <div className="relative flex items-center justify-between gap-2.5">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <Icons.clear className="h-4 w-4 text-white" aria-hidden="true" />
